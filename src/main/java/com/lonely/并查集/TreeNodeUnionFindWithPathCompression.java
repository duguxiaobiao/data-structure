 ...
 * @author ztkj-hzb
 * @Date 2019/8/8 11:41
 * @Description 基于树节点实现并查集, 基于树的深度优化,以及路径压缩
 */
public class TreeNodeUnionFindWithPathCompression implements UnionFind {
    ...
    public TreeNodeUnionFindWithPathCompression(int size) {
        if (size < 0) {
    ...
    private int getRootId(int p) {
        ...
        int currId = p;
        while (currId != this.parentId(currId)) {
            this.ids[currId] = parentId(parentId(currId));
            currId = this.parentId(currId);
        }
        return currId;
    }
